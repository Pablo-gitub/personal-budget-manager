...
import main.charts.ChartBuilder;
...
public class ExpenditureScene extends BaseScene{
    ...
    protected void updateCenter() {
        ...
        if (transactionChart != null && transactionChart.size() >= 0 && selectedUser != null) {
            ...
            try {
                ...
                pie = ChartBuilder.builderChartList(bankName, transactionChart, firstDate, secondDate);
            ...
            // Display a message indicating the user to select an account or user
            final Text selectUserText = new Text("Select user");
            selectUserText.setFont(Font.font("Arial", FontWeight.BOLD, 20));
            centerLayout.getChildren().add(selectUserText);
        }
    ...
    protected void updateLeft() {
        ...
        if (transactionChart != null && transactionChart.size() >= 0 && selectedUser != null) {
            ...
            try {
                //once reading json will be fixed we can call a get-Transaction method and put it into the first element
                //once set button on the view for chose date start and date end we will be able to see the asked data
                area = ChartBuilder.areaChartBuilderList(transactionChart, firstDate, secondDate);
            } catch (ParseException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
            ...
            final Text selectUserText = new Text("Select user");
            selectUserText.setFont(Font.font("Arial", FontWeight.BOLD, 20));
            leftLayout.getChildren().add(selectUserText);
        }
     ...
     * */
    private void performBankTransaction(final String nameTransaction, final double amount, final String datetime) {
    ...
    private void performMoneyboxTransaction(final String nameTransaction, final String currency, final double amount, final String datetime) {
    ...
    private void performAssetTransaction(final String nameTransaction, final String assetName, final double amount, final String datetime) {
    ...
    private void performAssetCreationAndTransaction(final String assetName, final String symbol, final String nameTransaction, 
            final double amount, final String datetime) {
    ...
    private TableView<main.jsonfile.TransactionJson> createTransactionTableView() {
        updateTransactionTables();
        try {
            return ChartBuilder.buildTableList(transactionTable, firstDate, secondDate);
        } catch (ParseException e) {
            e.printStackTrace();
    ...
    private HBox createTableButtons() {
        final Button addAssetButton = new Button("Aggiungi Asset");
        ...
        button.setOnAction(event -> {
            ...
            final String nameTransaction = name.getText();
            final double amount = importo.getNumber().doubleValue();
            final TransactionJson newTransaction;

            if (set == 0) {
                if (type == 1) {
                    // Find the selected bank by name
                    BankAccountJson selectedBank = null;
                    for (final BankAccountJson bank : selectedUser.getBanks()) {
                    ...
                    // new transaction
                    newTransaction = new TransactionJson(nameTransaction, datetime.substring(0, 10), datetime.substring(11), amount);
                    if (selectedBank != null && selectedBank.validTransaction(newTransaction)) {
                        performBankTransaction(nameTransaction, amount, datetime);
                    }
                } else if (type == 2) {
                    // Find the selected Moneybox by name
                    MoneyboxAccountJson selectedMoneyBox = null;
                    for (final MoneyboxAccountJson box : selectedUser.getMoneyboxes()) {
                        if (tableName.equals(box.getName())) {
                            selectedMoneyBox = box;
                            break;
                        }
                    }
                    // new transaction
                    final String currency = currencyValue.getText();
                    newTransaction = new TransactionJson(nameTransaction, datetime.substring(0, 10), datetime.substring(11), amount, currency);
                    if (selectedMoneyBox != null && selectedMoneyBox.validTransaction(newTransaction)) {
                        performMoneyboxTransaction(nameTransaction, currency, amount, datetime);
                    }
                } else if (type == 3) {
                    // Find the selected investment account by name
                    InvestmentAccountJson selectedInvestmentAccount = null;
                    for (final InvestmentAccountJson inv : selectedUser.getInvestmentAccounts()) {
                        if (tableName.equals(inv.getNameInvestmentAccount())) {
                            selectedInvestmentAccount = inv;
                            break;
                        }
                    }
                    // Find the selected asset by name
                    final String assetName = assetChooser.getValue();
                    AssetJson selectedAsset = null;
                    for (final AssetJson asset : selectedInvestmentAccount.getAssets()) {
                        if (assetName.equals(asset.getAssetSymbol())) {
                            selectedAsset = asset;
                            break;
                        }
                    }
                    // new transaction
                    newTransaction = new TransactionJson(nameTransaction, datetime.substring(0, 10), datetime.substring(11), amount, selectedAsset.getAssetSymbol());
                    if (selectedAsset != null && selectedAsset.validTransaction(newTransaction)) {
                        performAssetTransaction(nameTransaction, assetName, amount, datetime);
                    }
                }
            } else if (set == 1 && type == 3) {
                final String assetName = nameNewAsset.getText();
                final String symbol = symbolNewAsset.getText();
    ...
    protected void updateRight() {
        ...
        if (transactionTable != null && transactionTable.size() >= 0 && selectedUser != null) {
            ...
            final TableView<main.jsonfile.TransactionJson> tableView = createTransactionTableView();
            //Button addAssetButton = createAddAssetButton();
            final HBox tableButtons = createTableButtons();
            final ObservableList<Node> children = tableButtons.getChildren();
            final Node compilingElement = children.get(0); // Accesses the first element in tableButtons
            final Node addingElement = children.get(1); // Accesses the second element in tableButtons
            ...
            final Text selectUserText = new Text("Select user");
            selectUserText.setFont(Font.font("Arial", FontWeight.BOLD, 20));
            rightLayout.getChildren().add(selectUserText);
        }
